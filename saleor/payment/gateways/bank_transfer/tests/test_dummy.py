...
def setup_dummy_gateway(settings):
    settings.PLUGINS = ["saleor.payment.gateways.bank_transfer.plugin.BankTransferGatewayPlugin"]
    return settings
...
def test_authorize_gateway_error(payment_dummy, monkeypatch):
    monkeypatch.setattr("saleor.payment.gateways.bank_transfer.dummy_success", lambda: False)
    with pytest.raises(PaymentError):
        txn = gateway.authorize(
            payment=payment_dummy,
            token="Fake",
